...
> rasterize_forward_sum_gabor_tensor(
    ...
    rasterize_forward_sum_gabor<<<tile_bounds_dim3, block_dim3>>>(
        ...
        (float3 *)colors.contiguous().data_ptr<float>(),
        opacities.contiguous().data_ptr<float>(),
        /* kernel expects freqs_x, freqs_y, then weights */
...
// ============================================================
// 4-channel Gabor forward tensor wrapper
// ============================================================
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>
rasterize_forward_sum_gabor_4ch_tensor(
    const std::tuple<int, int, int> tile_bounds,
    const std::tuple<int, int, int> block,
    const std::tuple<int, int, int> img_size,
    const torch::Tensor &gaussian_ids_sorted,
    ...
    rasterize_forward_sum_gabor_4ch<<<tile_bounds_dim3, block_dim3>>>(
        tile_bounds_dim3,
        img_size_dim3,
        gaussian_ids_sorted.contiguous().data_ptr<int32_t>(),
        (int2 *)tile_bins.contiguous().data_ptr<int>(),
...
// ============================================================
// 4-channel Gabor backward tensor wrapper
// ============================================================
std::tuple<
    torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor,
    torch::Tensor, torch::Tensor, torch::Tensor
>
rasterize_backward_sum_gabor_4ch_tensor(
    const unsigned img_height,
    const unsigned img_width,
    const unsigned BLOCK_H,
    const unsigned BLOCK_W,
    ...
    TORCH_CHECK(colors.ndimension() == 2 && colors.size(1) == 4,
                "colors must have dimensions (num_points, 4)");
    TORCH_CHECK(background.ndimension() == 1 && background.size(0) == 4,
                "background must be 4D vector");
    ...
    rasterize_backward_sum_gabor_4ch_kernel<<<tile_bounds_d, block>>>(
        tile_bounds_d,
        img_size,
        gaussians_ids_sorted.contiguous().data_ptr<int32_t>(),
        (int2*)tile_bins.contiguous().data_ptr<int>(),
    ...
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        AT_ERROR("CUDA kernel launch error in rasterize_backward_sum_gabor_4ch_kernel: ",
                 cudaGetErrorString(err));
    }
