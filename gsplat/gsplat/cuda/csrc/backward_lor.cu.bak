...
__global__ void nd_rasterize_backward_sum_kernel(
    ...
    __shared__ float3 xy_opacity_batch[BLOCK_SIZE];
    __shared__ float3 conic_batch[BLOCK_SIZE];
    //__shared__ float rgbs_batch[BLOCK_SIZE];
    __shared__ float4 rgbs_batch[BLOCK_SIZE];

    // df/d_out for this pixel
    // const float* v_out = &v_output[pix_id];
    ...
    for (int b = 0; b < num_batches; ++b) {
        ...
        for (int t = max(0,batch_end - warp_bin_final); t < batch_size; ++t) {
            ...
            // float v_rgb_local[MAX_REGISTER_CHANNELS];
            // float v_rgb_local[channels];
            ...
            if (warp.thread_rank() == 0) {
                ...
                // for (int ch = 0; ch < channels; ++ ch){
                   // atomicAdd(v_rgb_ptr + channels * g + ch, v_rgb_local[ch]);
                // }
