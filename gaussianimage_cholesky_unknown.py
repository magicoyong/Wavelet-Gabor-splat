...
class GaussianImage_Cholesky_EA(nn.Module):
    ...
    def forward(self):
        ...
        features_split = torch.split(self.get_features, 3, dim=1)  

        out_imgs = []
        for fs in features_split:
            out = rasterize_gaussians_sum(
                self.xys, depths, self.radii, conics, num_tiles_hit,
                fs, self._opacity, self.H, self.W, self.BLOCK_H, self.BLOCK_W, return_alpha=False
            )
            out_imgs.append(out)

        out_img = torch.cat(out_imgs, dim=2)
    ...
    def forward_quantize(self):
        ...
        features_split = torch.split(features, 3, dim=1)  
